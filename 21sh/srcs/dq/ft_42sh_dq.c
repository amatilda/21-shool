...
register t_in_42sh *list, register unsigned char litter)
{
	...
	tmp + parser->count)) == 0 || litter == PARSING_FALTURE)
...
register t_in_42sh *list)
{
	...
	else
	{
		...
		tmp + list->count)) != 0 && litter != PARSING_FALTURE)
			ft_42sh_dq_cut(array, list);
	}
	ft_42sh_msg_change(array, (litter == PARSING_FALTURE ? 0 : litter),
	array->dq.dquote);
	if (litter == 0 || litter == PARSING_FALTURE)
