...
{
	chdir(array->pwd.prev_path.buff);
	if (ft_42sh_pipe_test_fd_dsp(array, STDOUT_FILENO, "cd") == 0)
		return ;
	ft_write_buffer_str(&array->out, array->pwd.path.buff,
	array->pwd.path.len);
...
register t_string *path, register char *str, register size_t n)
{
	if (chdir(str) != 0)
	{
		ft_42sh_dsp_err_msg_add(array, MSG_CD_NOT_VALID_42SH, str);
		ft_free(str);
		return ;
	}
...
register char **lp_arg)
{
	...
	if (tempos != 1)
		return (ft_42sh_dsp_err_msg(array, MSG_CD_42SH));
