...
/*   Updated: 2019/11/02 15:11:55 by jkrypto          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../incs/ft_itoa.h"
		...
		uint_fast8_t base, uint_fast8_t option)
{
	...
	itoa.buffer = buffer;
	itoa.option = option;
	itoa.alphabet = (itoa.option & ITOA_UPPER) ? ITOA_UP_ABC : ITOA_LOW_ABC;
	fn_prev(&itoa);
	if ((itoa.option & ITOA_NOT_ONE) && itoa.value < itoa.base)
	{
		itoa.buffer++[0] = 0x30;
